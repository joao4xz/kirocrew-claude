...
class AcpClient:
    ...
    def __init__(
        ...
        # Fork: the initial seed of settings.local.json (pinning self._model
        # when _model_via_env is set) now happens via upstream's own
        # _write_claude_local_settings seam call on the primary spawn path
        # (and its model-substitution-retry re-seed) — both run after
        # _model_via_env is already fixed above, so no separate __init__-time
        # call is needed here.
